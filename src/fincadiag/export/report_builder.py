...
def _build_obj3_main_text(
    ...
    if serial.get("available", False):
        lines.append(
            f"Payload publicable via gateway: confianza parser={_format_value(serial.get('parser_confidence_average'), 3)}, "
            f"identidad confirmada={serial.get('identity_confirmed_count', 0)}, "
            f"probable={serial.get('identity_probable_count', 0)}, sin confirmar={serial.get('identity_unconfirmed_count', 0)}."
        )
        lines.append(
            f"Microeventos suprimidos: {serial.get('suppressed_noise_event_count', 0)} ruido, "
            f"{serial.get('suppressed_retained_state_event_count', 0)} estado retenido (impactan conteo de mensajes cow_event)."
        )
    if int(correlation.get("matched_events", 0) or 0) > 0:
        lines.append(
            f"Correlacion serial-red: {correlation.get('matched_events', 0)} coincidencias, "
            f"eta={_format_value(correlation.get('eta_extraccion'), 2)}%, "
            f"desfase_medio={_format_value(correlation.get('desfase_medio_ms'))} ms — base para sincronizacion en correlation_summary."
        )
    if field_validation.get("available", False):
        lines.append(
            f"Validacion de campo disponible: {field_validation.get('observed_cows_count', 0)} vacas, "
            f"{field_validation.get('quick_id_count', 0)} IDs rapidas — habilita field_validation_summary en payload del gateway."
        )
    if pcap.get("available", False):
        lines.append("PCAP parseado disponible: permite justificar politica de red y control de publicacion en la pasarela.")
    if not lines:
        lines.append("Sin artefactos suficientes para un payload de gateway completo; la sesion aporta solo contexto minimo.")
    return "\n".join(f"- {line}" for line in lines)


def _build_obj4_context_text(
    baseline: dict,
    pcap: dict,
    correlation: dict,
) -> str:
    lines = []
    lat = float(baseline.get("lat_media") or 0.0)
    jitter = float(baseline.get("jitter_ms") or 0.0)
    loss = float(baseline.get("packet_loss") or 0.0)
    nodos = int(baseline.get("nodos_totales") or 0)
    lines.append(
        f"Baseline de red: latencia media={_format_value(lat)} ms, jitter={_format_value(jitter)} ms, "
        f"perdida={_format_value(loss, 2)}%, {nodos} nodos ARP."
    )
    if pcap.get("available", False):
        general = pcap.get("general", {})
        mc = float(general.get("multicast_pct") or 0.0)
        bc = float(general.get("broadcast_pct") or 0.0)
        lines.append(
            f"Red durante captura: multicast={_format_value(mc, 2)}%, broadcast={_format_value(bc, 2)}% "
            f"— condiciones de fondo sobre las cuales se ejecutan los escenarios de resiliencia."
        )
    if int(correlation.get("matched_events", 0) or 0) > 0:
        lines.append(
            f"Sincronismo serial-red confirmado en {correlation.get('matched_events', 0)} coincidencias "
            f"— indica que gateway y bus estuvieron operativos a la vez en esta muestra."
        )
    if loss > 1.0:
        lines.append(
            f"AVISO: perdida baseline={_format_value(loss, 2)}%; puede influir en MTTR medido en Objetivo 4."
        )
    else:
        lines.append("Red base estable (perdida <= 1%), adecuada como referencia para escenarios de resiliencia (broker/network/kill).")
    return "\n".join(f"- {line}" for line in lines)
...
def build_technical_report(
    ...
    executive_takeaway = _build_executive_takeaway(operation_mode, serial, pcap, correlation, field_validation)
    if objetivo == 3:
        _obj_label = "Objetivo 3 (Pasarela IoT)"
        _section1_evidence = _build_obj3_main_text(serial, pcap, correlation, field_validation)
        _section3_label = "Resumen principal para Objetivo 3 (pasarela)"
        _section3_content = obj3_bridge_text
    elif objetivo == 4:
        _obj_label = "Objetivo 4 (Resiliencia del gateway)"
        _section1_evidence = _build_obj4_context_text(baseline, pcap, correlation)
        _section3_label = "Contexto de red para Objetivo 4 (resiliencia)"
        _section3_content = _build_obj4_context_text(baseline, pcap, correlation)
    else:
        _obj_label = "Objetivo 1"
        _section1_evidence = obj1_evidence
        _section3_label = "Apertura hacia Objetivo 3 / gateway"
        _section3_content = obj3_bridge_text
...
- Cobertura estimada de campo: {_format_value(field_validation.get('field_coverage_pct'))}% de {field_validation.get('expected_herd_cows', 38)} vacas
- Calidad de validacion presencial: {field_validation.get('field_validation_quality', 'N/D')} ({field_validation.get('field_validation_scope', 'N/D')})
- Alineacion parser-campo: {field_validation.get('parser_field_alignment', 'N/D')}
- Parser lee mayoria del campo observado: {'Si' if field_validation.get('parser_reads_majority_of_field') else 'No'}
...
Rol analitico en {_obj_label}: {obj1_role}
Confianza de uso para {_obj_label}: {obj1_confidence}

1. Lectura ejecutiva de la muestra
- Sintesis rapida: {executive_takeaway}
- Aporte principal al indicador del {_obj_label}: {_section1_evidence}
...
def build_human_report(
    ...
    if objetivo == 3:
        _obj_label = "Objetivo 3 (Pasarela IoT)"
        _section1_evidence = _build_obj3_main_text(serial, pcap, correlation, field_validation)
        _section3_label = "Lo que esta muestra aporta al Objetivo 3 (gateway)"
        _section3_content = obj3_bridge_text
    elif objetivo == 4:
        _obj_label = "Objetivo 4 (Resiliencia del gateway)"
        _section1_evidence = _build_obj4_context_text(baseline, pcap, correlation)
        ...
        _obj_label = "Objetivo 1"
        _section1_evidence = obj1_evidence
        _section3_label = "Lo que esta muestra abre hacia Objetivo 3"
        _section3_content = obj3_bridge_text
    operation_mode_label = {
        "ordeno_completo": "Ordeño completo",
        "telemetria_collar": "Telemetría de collar",
    ...
    if field_validation.get("available", False):
        field_validation_text = (
            ...
            f"con calidad {field_validation.get('field_validation_quality', 'N/D')} "
            f"y alcance {field_validation.get('field_validation_scope', 'N/D')}; "
        ...
        field_validation_text_2 = (
            ...
            f"Alineacion parser-campo: {field_validation.get('parser_field_alignment', 'N/D')}. "
...
1. Lectura ejecutiva
- {executive_takeaway}
- Aporte al indicador del {_obj_label}: {_section1_evidence}
...
def build_executive_summary(
    ...
    """Informe ejecutivo. El parametro objetivo (1, 3 o 4) cambia titulo y secciones.
    ...
    _titles = {
        1: "OBJETIVO 1: CARACTERIZACION FORENSE DEL SISTEMA",
        3: "OBJETIVO 3: PASARELA IoT (GATEWAY MQTT/TLS)",
        4: "OBJETIVO 4: RESILIENCIA DEL GATEWAY",
    }
    _sec2_labels = {
        1: "CARACTERIZACION FORENSE (OBJETIVO 1)",
        3: "PREPARACION PARA GATEWAY (OBJETIVO 3)",
        4: "CONTEXTO DE RED PARA RESILIENCIA (OBJETIVO 4)",
    }
    _sec7_labels = {
        1: "CONCLUSIONES OBJETIVO 1",
        3: "CONCLUSIONES OBJETIVO 3",
        4: "CONCLUSIONES OBJETIVO 4",
    }
    _obj_title = _titles.get(objetivo, _titles[1])
    _sec2 = _sec2_labels.get(objetivo, _sec2_labels[1])
    _sec7 = _sec7_labels.get(objetivo, _sec7_labels[1])
    return f"""
================================================================================
INFORME EJECUTIVO - {_obj_title}
