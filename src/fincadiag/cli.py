...
def build_batch_run_name(root_dirs: list[Path], requested_name: str | None = None, objetivo: int = 1) -> str:
    if requested_name:
        return normalize_run_name(requested_name)
    if objetivo in (3, 4):
        return f"Etapa_Obj{objetivo}"
    if len(root_dirs) == 1:
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--objetivo", type=int, choices=[1, 3, 4], default=1,
                        help="Objetivo del TFG para enmarcar los informes: 1=caracterizacion forense, 3=pasarela IoT, 4=resiliencia (default: 1)")
...
def build_session_summary(
    ...
    summary = {
        ...
        "cobertura_campo_pct": field_validation.get("field_coverage_pct", 0.0) if field_validation.get("available", False) else "",
        "calidad_validacion_campo": field_validation.get("field_validation_quality", "") if field_validation.get("available", False) else "",
        "alcance_validacion_campo": field_validation.get("field_validation_scope", "") if field_validation.get("available", False) else "",
        "alineacion_parser_campo": field_validation.get("parser_field_alignment", "") if field_validation.get("available", False) else "",
        "parser_lee_mayoria_campo": field_validation.get("parser_reads_majority_of_field", False) if field_validation.get("available", False) else "",
...
def process_session(session: dict, args) -> dict:
    ...
    if field_validation.get("available"):
        ...
        print(f"     Calidad validacion campo: {field_validation.get('field_validation_quality', 'N/D')}")
        print(f"     Alineacion parser-campo: {field_validation.get('parser_field_alignment', 'N/D')}")
...
def main() -> None:
    ...
    run_name = build_batch_run_name(root_dirs, args.run_name, objetivo=args.objetivo)
