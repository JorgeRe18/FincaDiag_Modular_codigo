...
VALIDATION_LEGACY_DOCX_DIR = PROJECT_ROOT / "vali_presencial_SG"
VALIDATION_DOCX_DIRS = (VALIDATION_DOCX_DIR, VALIDATION_LEGACY_DOCX_DIR)
...
EXPECTED_HERD_COWS = 38
FIELD_COMPLETE_MIN_COWS = 30
FIELD_PARTIAL_MIN_COWS = 10
PARSER_MAJOR_FIELD_COVERAGE_RATIO = 0.80
PARSER_ALIGNED_MIN_RATIO = 0.80
PARSER_ALIGNED_MAX_RATIO = 1.20
...
def empty_field_validation_summary() -> dict:
    return {
        ...
        "expected_herd_cows": EXPECTED_HERD_COWS,
        "field_coverage_pct": 0.0,
        "field_validation_scope": "sin_validacion",
        "field_validation_quality": "no_disponible",
        "field_validation_quality_reason": "",
        "parser_field_alignment": "sin_base",
        "parser_reads_majority_of_field": False,
        "parser_majority_threshold_ratio": PARSER_MAJOR_FIELD_COVERAGE_RATIO,
        "records": [],
    }
...
def _load_validation_rows() -> list[dict]:
    ...
    for docx_dir in VALIDATION_DOCX_DIRS:
        if not docx_dir.exists():
            continue
        for docx_path in sorted(docx_dir.glob("validacion_visita_*.docx")):
...
def _build_field_quality_summary(
    observed_cows_count: int,
    records_in_window_count: int,
    parser_event_count: int,
    overlap_seconds: float,
) -> dict:
    field_coverage_pct = round((observed_cows_count / EXPECTED_HERD_COWS) * 100.0, 2) if EXPECTED_HERD_COWS else 0.0
    parser_ratio = round(parser_event_count / observed_cows_count, 3) if observed_cows_count else 0.0
    parser_reads_majority = bool(observed_cows_count and parser_ratio >= PARSER_MAJOR_FIELD_COVERAGE_RATIO)

    if observed_cows_count >= FIELD_COMPLETE_MIN_COWS:
        field_scope = "completa_hato"
    elif observed_cows_count >= FIELD_PARTIAL_MIN_COWS:
        field_scope = "parcial_representativa"
    elif observed_cows_count > 0:
        field_scope = "minima_exploratoria"
    else:
        field_scope = "sin_validacion"

    if not observed_cows_count:
        alignment = "sin_base"
    elif parser_ratio < PARSER_ALIGNED_MIN_RATIO:
        alignment = "subconteo_parser"
    elif parser_ratio > PARSER_ALIGNED_MAX_RATIO:
        alignment = "sobreconteo_parser"
    else:
        alignment = "alineado"

    if field_scope == "completa_hato" and alignment == "alineado" and records_in_window_count >= observed_cows_count:
        quality = "alta"
        reason = "validacion amplia y conteo del parser alineado con campo"
    elif field_scope in {"completa_hato", "parcial_representativa"} and parser_reads_majority:
        quality = "media"
        reason = "validacion util para contraste, aunque no cierra todo el hato o presenta diferencia de conteo"
    elif observed_cows_count > 0:
        quality = "baja"
        reason = "validacion parcial o desalineada; usar como evidencia auxiliar, no como cobertura total"
    else:
        quality = "no_disponible"
        reason = "sin vacas observadas dentro de la ventana de captura"

    return {
        "expected_herd_cows": EXPECTED_HERD_COWS,
        "field_coverage_pct": field_coverage_pct,
        "field_validation_scope": field_scope,
        "field_validation_quality": quality,
        "field_validation_quality_reason": reason,
        "parser_field_alignment": alignment,
        "parser_reads_majority_of_field": parser_reads_majority,
        "parser_majority_threshold_ratio": PARSER_MAJOR_FIELD_COVERAGE_RATIO,
    }
...
def build_field_validation_summary(session: dict, capture_dir: Path, serial: dict) -> dict:
    ...
    records_in_window_count = len(records_in_window)
    field_rate_denominator = records_in_window_count or observed_cows_count
    quality_summary = _build_field_quality_summary(
        observed_cows_count=observed_cows_count,
        records_in_window_count=records_in_window_count,
    ...
    summary.update(
        {
            ...
            "records_in_capture_window": records_in_window_count,
            "observed_cows_count": observed_cows_count,
            ...
            "field_id_success_rate": round(id_success_count / field_rate_denominator, 3) if field_rate_denominator else 0.0,
            "field_flow_visible_rate": round(flow_visible_count / field_rate_denominator, 3) if field_rate_denominator else 0.0,
