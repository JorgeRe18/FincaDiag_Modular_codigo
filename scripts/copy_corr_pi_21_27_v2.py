...
Version 2: crea directorios padres primero via SSH, luego sube via SFTP."""
...
# Crear directorios via SSH (mkdir -p) y subir archivos
for s in sessions_to_sync:
    try:
        # Crear directorio completo
        stdin, stdout, stderr = c.exec_command(f"mkdir -p {s['pi_dir']}")
        stdout.read()
        err = stderr.read().decode()
        if err.strip():
            print(f"  mkdir warn: {err.strip()}")

        # Subir archivo
        remote_path = s['pi_dir'] + '/correlation_summary.json'
        sftp.put(s['local_path'], remote_path)
        print(f"  OK: {s['visit']}/{s['session']}")
...
for s in sessions_to_sync:
    ...
    if err:
        print(f"  {s['visit']}/{s['session']}: ERROR {err}")
    else:
